...
def main():
    ...
    print(f"{ctime()} | [Main PID: {main_pid}] [Main TID: {main_tid}] === เริ่มระบบจำลองตู้กาแฟแบบ Multi-Process ===")
    start_time = time()

    processes = []
    # ลูปสร้าง Process ขึ้นมารับออเดอร์ลูกค้าแต่ละคน
    for customer in queue:
        process = multiprocessing.Process(target=make_coffee, args=(customer,))
        processes.append(process)
        process.start()
        
    # รอให้ Process ทั้งหมดทำงานเสร็จ
    for p in processes:
        p.join()
