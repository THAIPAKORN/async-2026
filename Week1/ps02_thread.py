...
def make_coffee(customer_name):
    ...
    sum(i * i for i in range(10000000))  # จำลองงาน CPU-bound เล็กน้อย และรอ 5 วินาที
...
def main():
    ...
    print(f"{ctime()} | [Main PID: {main_pid}] [Main TID: {main_tid}] === เริ่มระบบจำลองลูกค้าแบบ Multi-Thread ===")
    start_time = time()
    ...
    for customer in queue:
        # เราจะมองหนึ่ง Thread ผ่านพารามิเตอร์ name= ได้เพื่อให้ใช้โอ้อ้างนี้
        t = threading.Thread(
            target=make_coffee,
            args=(customer,),
    ...
    # ใช้ psutil วัดการกิน RAM
    process = psutil.Process(os.getpid())
    mem_mb = process.memory_info().rss / (1024 * 1024)

    print(f"\n[สรุปผล Multi-Thread]")
    print(f"เวลาที่ใช้จริง (Wall Time): {duration:0.2f} วินาที")
    print(f"เวลาที่ CPU ใช้งานผลรวมจริง (CPU Time): {cpu_duration:0.4f} วินาที")
